...
// Curated Genre Metadata & Art Direction Colors
const GENRE_METADATA = {
  Action: { from: "#f43f5e", to: "#f97316", desc: "High-octane battles, martial arts, and explosive combat sequences." },
  Adventure: { from: "#0ea5e9", to: "#14b8a6", desc: "Epic journeys, uncharted worlds, and courageous quests into the unknown." },
  Comedy: { from: "#f59e0b", to: "#84cc16", desc: "Hilarity, absurdity, and lighthearted moments to brighten your day." },
  Drama: { from: "#8b5cf6", to: "#4f46e5", desc: "Deep emotional stories, complex relationships, and compelling human conflicts." },
  Ecchi: { from: "#f43f5e", to: "#ec4899", desc: "Playful humor, fanservice, and lighthearted romantic antics." },
  Fantasy: { from: "#6366f1", to: "#22d3ee", desc: "Magic, mythical realms, dragons, and worlds beyond imagination." },
  Horror: { from: "#991b1b", to: "#450a0a", desc: "Chilling suspense, terrifying encounters, and dark psychological dread." },
  "Mahou Shoujo": { from: "#ec4899", to: "#8b5cf6", desc: "Magical girls, transformation sequences, and cosmic battles for justice." },
  Mecha: { from: "#3b82f6", to: "#1d4ed8", desc: "Giant robots, futuristic warfare, and pilot conflicts." },
  Music: { from: "#a855f7", to: "#ec4899", desc: "Rhythmic melodies, idol performances, and passionate musical journeys." },
  Mystery: { from: "#64748b", to: "#0f172a", desc: "Perplexing puzzles, detective investigations, and secrets waiting to be solved." },
  Psychological: { from: "#475569", to: "#1e1b4b", desc: "Mind games, moral dilemmas, and deep explorations of the human psyche." },
  Romance: { from: "#ec4899", to: "#a855f7", desc: "Heartfelt love stories, blossoming relationships, and tender emotions." },
  "Sci-Fi": { from: "#06b6d4", to: "#3b82f6", desc: "Futuristic technology, space exploration, cybernetics, and dystopian futures." },
  "Slice of Life": { from: "#10b981", to: "#06b6d4", desc: "Warm everyday moments, peaceful routines, and comforting coming-of-age stories." },
  Sports: { from: "#22c55e", to: "#eab308", desc: "Intense competition, teamwork, and athletic passion." },
  Supernatural: { from: "#a855f7", to: "#f43f5e", desc: "Spirits, occult mysteries, and powers beyond mortal realm." },
  Thriller: { from: "#dc2626", to: "#7f1d1d", desc: "Edge-of-your-seat suspense, high stakes, and relentless tension." },
};
...
const Genre = () => {
  ...
  useEffect(() => {
    ...
    document.title = formattedName ? `${formattedName} Anime — OtakuStreams` : "Genre Anime — OtakuStreams";
  }, [name]);
  ...
  // Parse Genre Name from URL slug
  const genreSlug = (name || "action").toLowerCase();
  const matchedGenreObj = ALL_GENRES.find((g) => g.slug === genreSlug) || {
    slug: genreSlug,
    label: name ? name.split("-").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ") : "Genre",
  };
  const genreTitle = matchedGenreObj.label;
  const genreMeta = GENRE_METADATA[genreTitle] || {
    from: "#6366f1",
    to: "#22d3ee",
    desc: `Discover top-rated ${genreTitle} anime series and movies.`,
  };

  // Reset internal page to 1 whenever genre or filters change
  useEffect(() => {
    setPage(1);
  }, [genreSlug, sort, format, status, year]);
  ...
  useEffect(() => {
    ...
    const loadGenreData = async () => {
      ...
      try {
        const res = await fetchgenres(genreSlug, page, {
          sort,
        ...
        if (isMounted) setError(err.message || "We couldn't load this genre right now.");
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  }, [genreSlug, page, sort, format, status, year, fetchgenres]);
  ...
  // Format Labels for Summary Line
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans selection:bg-primary/20 selection:text-primary relative">
      ...
      <main className="flex-1 w-full pt-[68px] pb-16 lg:pb-24">
              ...
              {ALL_GENRES.map((g) => {
                ...
                return (
                  <Link
                    ...
                    className={`shrink-0 snap-start px-4 h-11 rounded-full text-sm font-semibold flex items-center justify-center transition-all duration-150 cursor-pointer min-w-[44px] ${
                      isActive
          ...
          <div className="mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-10 flex items-center justify-between gap-4">
            ...
            <div className="hidden lg:flex items-center gap-3">
                  ...
                  <div className="absolute left-0 mt-2 w-48 rounded-2xl bg-surface border border-border shadow-lift p-1.5 z-40 space-y-1 font-sans">
                    {SORT_OPTIONS.map((opt) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          updateParams({ sort: opt.id, page: 1 });
                  ...
                  <div className="absolute left-0 mt-2 w-44 rounded-2xl bg-surface border border-border shadow-lift p-1.5 z-40 space-y-1 font-sans">
                    {FORMAT_OPTIONS.map((opt) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          updateParams({ format: opt.id, page: 1 });
                          setActivePopover(null);
                        }}
                        className={`w-full h-10 px-3 rounded-xl text-xs font-medium flex items-center justify-between text-left transition-colors cursor-pointer ${
                          format === opt.id
                  ...
                  <div className="absolute left-0 mt-2 w-48 rounded-2xl bg-surface border border-border shadow-lift p-1.5 z-40 space-y-1 font-sans">
                    {STATUS_OPTIONS.map((opt) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          updateParams({ status: opt.id, page: 1 });
                          setActivePopover(null);
                        }}
                        className={`w-full h-10 px-3 rounded-xl text-xs font-medium flex items-center justify-between text-left transition-colors cursor-pointer ${
                          status === opt.id
                  ...
                  <div className="absolute left-0 mt-2 w-44 max-h-64 overflow-y-auto rounded-2xl bg-surface border border-border shadow-lift p-1.5 z-40 space-y-1 font-sans no-scrollbar">
                    {YEAR_OPTIONS.map((opt) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          updateParams({ year: opt.id, page: 1 });
                          setActivePopover(null);
                        }}
                        className={`w-full h-10 px-3 rounded-xl text-xs font-medium flex items-center justify-between text-left transition-colors cursor-pointer ${
                          year === opt.id
              ...
              {activeFilterCount > 0 && (
                <button
                  type="button"
                  onClick={() => updateParams({ sort: "popularity", format: "all", status: "all", year: "all", page: 1 })}
        ...
        {/* 7. Collection Grid / List */}
        <section id="genre-collection" className="mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-10 pt-6 min-h-[420px]">
          {/* Skeleton Load State */}
          {loading && (
            <div
              className={`w-full ${
            ...
            <div className="flex flex-col items-center justify-center min-h-[420px] text-center p-8 bg-surface/30 rounded-2xl border border-border">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="px-5 h-11 rounded-full bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition-colors"
                >
                  Try again
                </button>
                <Link
          ...
          {!loading && !error && animeList.length === 0 && (
            <div className="flex flex-col items-center justify-center min-h-[420px] text-center p-8 bg-surface/30 rounded-2xl border border-border">
              ...
              <p className="text-sm text-muted-foreground max-w-md mb-6 font-sans">
                Try loosening your format, status, or year criteria to see more titles.
              </p>
              <button
                type="button"
                onClick={() => updateParams({ sort: "popularity", format: "all", status: "all", year: "all", page: 1 })}
          ...
          <div className="w-full bg-surface border-t border-border rounded-t-3xl p-5 space-y-6 max-h-[85dvh] overflow-y-auto pb-[max(1.5rem,env(safe-area-inset-bottom))] shadow-lift text-left">
            ...
            <div className="pt-4 border-t border-border flex items-center gap-3 sticky bottom-0 bg-surface">
              ...
              <button
                type="button"
                onClick={() => {
                  updateParams({ ...draftFilters, page: 1 });
