...
// Official 12 Studio Set & Art Direction Metadata
const PRODUCER_TILES = [
  { slug: "toei-animation", name: "Toei Animation", from: "#f43f5e", to: "#f97316", desc: "The house that built shonen — decades of long-running legends and cinema-scale spectacle." },
  { slug: "mappa", name: "MAPPA", from: "#6366f1", to: "#22d3ee", desc: "High-octane action, cinematic intensity, and industry-defining animation prowess." },
  { slug: "ufotable", name: "ufotable", from: "#8b5cf6", to: "#4f46e5", desc: "Unmatched digital compositing, breathtaking choreography, and visual perfection." },
  { slug: "wit-studio", name: "Wit Studio", from: "#0ea5e9", to: "#14b8a6", desc: "Dynamic momentum, visceral action, and handcrafted artistic flair." },
  { slug: "bones", name: "Bones", from: "#f59e0b", to: "#84cc16", desc: "Fluidity in motion, character-driven storytelling, and legendary fight sequences." },
  { slug: "madhouse", name: "Madhouse", from: "#dc2626", to: "#7f1d1d", desc: "Decades of groundbreaking masterpieces, dark thrillers, and anime history." },
  { slug: "kyoto-animation", name: "Kyoto Animation", from: "#22c55e", to: "#eab308", desc: "Masterful emotional storytelling, unmatched character acting, and luminous art." },
  { slug: "a-1-pictures", name: "A-1 Pictures", from: "#ec4899", to: "#a855f7", desc: "Blockbuster adaptations, vibrant worlds, and modern fan-favorite spectacles." },
  { slug: "production-i-g", name: "Production I.G", from: "#06b6d4", to: "#3b82f6", desc: "Pioneers of sci-fi, realistic sports drama, and technical excellence." },
  { slug: "sunrise", name: "Sunrise", from: "#a855f7", to: "#f43f5e", desc: "The titan of mecha anime, legendary epics, and timeless sci-fi sagas." },
  { slug: "studio-pierrot", name: "Studio Pierrot", from: "#10b981", to: "#06b6d4", desc: "Icons of global anime culture, epic fantasy series, and iconic battles." },
  { slug: "shaft", name: "Shaft", from: "#64748b", to: "#0f172a", desc: "Avant-garde direction, signature head tilts, and surreal visual design." }
];
...
const Producer = () => {
  ...
  // Parse Studio Slug from URL
  const studioSlug = (name || "toei-animation").toLowerCase();
  const matchedStudioObj = PRODUCER_TILES.find((s) => s.slug === studioSlug) || {
    slug: studioSlug,
    name: name ? name.split("-").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ") : "Studio",
    from: "#6366f1",
    to: "#a855f7",
    desc: "Production studio anime collection.",
  };

  useEffect(() => {
    document.title = matchedStudioObj?.name ? `${matchedStudioObj.name} Anime — OtakuStreams` : "Studio Anime — OtakuStreams";
  }, [matchedStudioObj]);

  const studioName = matchedStudioObj.name;
  ...
  // Reset page to 1 when studio or server sort changes
  useEffect(() => {
    setPage(1);
  }, [studioSlug, sort]);
  ...
  useEffect(() => {
    ...
    const loadStudioData = async () => {
      ...
      try {
        const res = await fetchproducers(studioSlug, page, { sort });
        ...
        if (res) {
          ...
          throw new Error("Failed to load studio productions");
        }
      } catch (err) {
        console.error("Studio fetch error:", err);
        if (isMounted) setError(err.message || "We couldn't load this studio right now.");
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  }, [studioSlug, page, sort, fetchproducers]);

  // Client-side filtering for Format, Status, and Year per specification
  const filteredAnimeList = useMemo(() => {
    return rawAnimeList.filter((anime) => {
      // Format Filter
      if (format !== "all") {
        const animeFormat = (anime.type || "TV").toLowerCase();
        if (animeFormat !== format.toLowerCase()) return false;
      }
      // Status Filter
      if (status !== "all") {
        const rawStatus = (anime.status || "").toUpperCase();
        if (status === "airing" && rawStatus !== "RELEASING" && rawStatus !== "AIRING") return false;
        if (status === "finished" && rawStatus !== "FINISHED") return false;
        if (status === "upcoming" && rawStatus !== "NOT_YET_RELEASED" && rawStatus !== "UPCOMING") return false;
      }
      // Year Filter
      if (year !== "all") {
        const animeYear = anime.year ? anime.year.toString() : null;
        if (animeYear !== year) return false;
      }
      return true;
    });
  }, [rawAnimeList, format, status, year]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans selection:bg-primary/20 selection:text-primary relative">
      ...
      <main className="flex-1 w-full pt-[68px] pb-16 lg:pb-24">
        {/* 3. Hero Section with Atmospheric Studio Wash & Scrim */}
        <section className="relative w-full overflow-hidden border-b border-border/60 py-10 sm:py-14 lg:py-16">
          {/* Atmospheric Studio Wash (25% opacity per spec) */}
          <div
            className="absolute inset-0 opacity-25 transition-all duration-700 pointer-events-none"
            style={{
              background: `radial-gradient(circle at 50% 30%, ${matchedStudioObj.from} 0%, ${matchedStudioObj.to} 50%, transparent 80%)`,
            }}
          />
          ...
          <div className="relative mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-10 space-y-3 sm:space-y-4 text-left">
            ...
            {!loading && filteredAnimeList.length > 0 && (
              <div className="text-xs sm:text-sm font-normal text-muted-foreground font-sans tabular-nums pt-1">
                <span>{filteredAnimeList.length} anime showing</span>
              </div>
            )}
          </div>
        </section>

        {/* 4. Studio Rail Navigation */}
        <section className="w-full border-b border-border/50 bg-background/50 backdrop-blur-sm">
          <div className="mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-10">
            ...
            >
              {PRODUCER_TILES.map((s) => {
                const isActive = s.slug === studioSlug;
                return (
                  <Link
                    key={s.slug}
          ...
          <div className="mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-10 flex items-center justify-between gap-4">
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              {activeFilterCount > 0 && (
                <button
                  type="button"
                  onClick={() => updateParams({ sort: "popularity", format: "all", status: "all", year: "all" })}
        ...
        <section id="producer-collection" className="mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-10 pt-6 min-h-[420px]">
          {/* Skeleton Load State */}
          {loading && (
            <div
              className={`w-full ${
          ...
          {!loading && error && (
            <div className="flex flex-col items-center justify-center min-h-[420px] text-center p-8 bg-surface/30 rounded-2xl border border-border">
              ...
              <h2 className="text-2xl font-bold font-display text-foreground mb-2">
                We couldn't load this studio right now.
              </h2>
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  to="/producer/toei-animation"
                  className="px-5 h-11 rounded-full bg-surface border border-border text-foreground font-semibold text-sm hover:bg-elevated transition-colors flex items-center justify-center"
                >
                  Browse all studios
                </Link>
              </div>
          ...
          {!loading && !error && filteredAnimeList.length === 0 && (
            <div className="flex flex-col items-center justify-center min-h-[420px] text-center p-8 bg-surface/30 rounded-2xl border border-border">
              ...
              <p className="text-sm text-muted-foreground max-w-md mb-6 font-sans">
                Try loosening your format, status, or year criteria to see more titles from {studioName}.
              </p>
              <button
                type="button"
                onClick={() => updateParams({ sort: "popularity", format: "all", status: "all", year: "all" })}
          ...
          {/* GRID VIEW */}
          {!loading && !error && filteredAnimeList.length > 0 && view === "grid" && (
          ...
          {/* LIST VIEW */}
          {!loading && !error && filteredAnimeList.length > 0 && view === "list" && (
          ...
          {!loading && !error && !data?.hasNextPage && filteredAnimeList.length > 0 && (
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end bg-overlay/70 backdrop-blur-sm lg:hidden animate-fade-in font-sans">
          ...
          <div className="w-full bg-surface border-t border-border rounded-t-3xl p-5 space-y-6 max-h-[85dvh] overflow-y-auto pb-[max(1.5rem,env(safe-area-inset-bottom))] shadow-lift text-left">
            ...
            <div className="pt-4 border-t border-border flex items-center gap-3 sticky bottom-0 bg-surface">
              <button
                type="button"
                onClick={() => setDraftFilters({ sort: "popularity", format: "all", status: "all", year: "all" })}
